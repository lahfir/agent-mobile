...
class FolderListViewModel: ObservableObject {
    ...
    private func setupSampleData() {
        ...
        folders = [
            Folder(name: "Happy Files", notes: sampleNotes, color: .gray),
            Folder(name: "Games", notes: Array(sampleNotes.prefix(2)), color: .pink),
            Folder(name: "Games", notes: Array(sampleNotes.prefix(2)), color: .gray)
        ]
    }
    
    func addFolder(name: String, color: Color) {
        let newFolder = Folder(name: name, notes: [], color: color)
